...
import { useProductStore } from '../stores/TodoStore';
...
async function increase(id) {
  const item = store.productsOnCart.find(element => element.id === id)
  item.quantity++
...
async function removeItem(id) {
  store.productsOnCart = store.productsOnCart.filter(product => product.id != id)
}
                  ...
                  <tr v-for="product in store.productsOnCart">
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="flex items-center">
                        <div class="flex-shrink-0 w-10 h-10">
                          <img class="w-full h-full object-contain rounded-md" :src="product.image" alt="">
                        </div>
                        <div class="ml-4">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="flex items-center justify-between w-24">
                        <button class="text-gray-500 hover:text-gray-700" @click="decrease(product.id)"
                        ...
                        <button class="text-gray-500 hover:text-gray-700" @click="increase(product.id)"
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="text-sm text-gray-900">${{ Math.round((product.quantity * product.price) * 1e12) / 1e12 }}
                      </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <button class="text-gray-500 hover:text-gray-700" @click="removeItem(product.id)"
                        aria-label="Remove item">
                        <svg class="w-4 h-4 fill-current" viewBox="0 0 20 20">
                          <path
                            d="M10 18a8 8 0 100-16 8 8 0 000 16zM6.3 6.3a1 1 0 011.4 0L10 8.6l2.3-2.3a1 1 0 011.4 1.4L11.4 10l2.3 2.3a1 1 0 01-1.4 1.4L10 11.4l-2.3 2.3a1 1 0 01-1.4-1.4L8.6 10 6.3 7.7a1 1 0 010-1.4z">
                          </path>
                        </svg>
                      </button>
                    </td>
        ...
        <div class="bg-white p-4 rounded-md shadow-md">
          ...
          <div class="mt-4">
            ...
            <button @click="checkout = !checkout"
              class="mt-4 w-full bg-gray-800 text-white text-center py-2 rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 focus:ring-offset-gray-50">
              Checkout
            </button>
          </div>
        </div>
      ...
      <div :class="checkout ? 'scale-100' : ''"
        ...
        <div class="flex items-center justify-center w-12 h-12 mx-auto mb-4 text-green-500 bg-green-100 rounded-full">
          <svg class="w-6 h-6 fill-current" viewBox="0 0 20 20">
            <path
              d="M17.707 4.293c-.391-.391-1.023-.391-1.414 0l-7.293 7.293-3.293-3.293c-.391-.391-1.023-.391-1.414 0-.391.391-.391 1.023 0 1.414l4 4c.195.195.451.293.707.293s.512-.098.707-.293l8-8c.391-.391.391-1.023 0-1.414z">
            </path>
          </svg>
      </div>
      <!-- Message -->
